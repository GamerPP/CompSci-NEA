#ifndef SRC_PIECES_ROOK_HPP
#define SRC_PIECES_ROOK_HPP
#include "../Piece.hpp"

class Rook : public Piece {
public:
    Rook(bool isWhite) {
        if (isWhite) name = "rook_white";
        else name = "rook_black";
    }
    ~Rook();
    ...
    bool isValidMove(Piece* board[8][8], int startX, int startY, int destX, int destY) override {
        ...
        else if (board[destX][destY]->name.back() != name.back()) return true;

        return false;
    }
};

#endif /* SRC_PIECES_ROOK_HPP */
